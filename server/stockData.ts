...
// Comprehensive Indian stock dataset with realistic technical indicators
// Covers NIFTY 50 + popular mid-caps across all sectors

const SECTORS = [
  "IT", "Banking", "Pharma", "Auto", "FMCG", "Energy", "Metals",
  "Telecom", "Infra", "Financial Services", "Chemicals", "Real Estate",
  "Media", "Cement", "Power"
];
...
function generateTechnicalIndicators(seed: StockSeed, dayOffset: number = 0): Stock {
  const s = seed.symbol.split("").reduce((a, c) => a + c.charCodeAt(0), 0) + dayOffset;

  const volatility = 0.02 + seededRandom(s * 1) * 0.04;
  const trend = (seededRandom(s * 2) - 0.45) * 0.08; // slightly bullish bias
  ...
  // RSI: biased toward swing-tradeable range
  ...
  if (macdHistogram > 0 && macdLine > macdSignal) {
    swingScore += 12;
    patterns.push("MACD Bullish Cross");
  } else if (macdHistogram < 0 && macdLine < macdSignal) {
    swingScore -= 8;
    patterns.push("MACD Bearish");
  }
  ...
  return {
    ...
    atr14: round(atr14, 2),
    adx14: round(adx14, 1),
    stochK: round(stochK, 1),
    stochD: round(Math.max(0, Math.min(100, stochD)), 1),
    vwap: round(vwap),
    obv,
    swingScore,
    signal,
...
export function getHistoricalData(symbol: string, days: number = 90): PricePoint[] {
  ...
  for (let i = days; i >= 0; i--) {
    ...
    const s = seed.symbol.split("").reduce((a, c) => a + c.charCodeAt(0), 0) + i;
